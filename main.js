...
function drawScreen(canvas_id)
{
  ...
  for (var k=30; k<45; k++) {
    for(var j=0; j<Math.PI*2*200; j++){
      ...
      ctx.fillRect(
        canvas.width/2 + (x * Math.max(Math.min(canvas.width/10, canvas.height/10),50) + delta+delta2+delta3)/1.5,
        canvas.height/2 + (y * Math.max(Math.min(canvas.width/10, canvas.height/10),50) + delta+delta2+delta3)/1.5,
